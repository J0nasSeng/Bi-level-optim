...
# config.normalize_fft = True
  ...
  config.window_size = 96  # m4_settings[m4_key]['window_size']           #Set automatically below depending on dataset
  config.fft_compression = 4  # m4_settings[m4_key]['fft_compression']    #Set automatically below depending on dataset
...
############################################### Choose micro arch search ###############################################
  ...
  skip_after_n = True      #choose wether or not to skip an episode after a number of steps
  n_skip = 90              #number of steps after which an episode is skipped if skip_after_n is True
...
########################################################################################################################
  ...
  for epoch in range(args.epochs):
    ...
    # training
    train_acc, train_obj = train_new(train_loader, test_loader, model, architect, criterion, optimizer, lr,model_1,use_smape,skip_after_n,n_skip)
